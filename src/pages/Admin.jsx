...
import bcrypt from 'bcryptjs'; // 🔥 암호화 라이브러리 추가

const { Option } = Select;

// --- 스타일 (기존 유지) ---
const AdminLayout = styled(Layout)`
  background: #111827; min-height: 100vh; padding: 20px; color: white;
`;
...
const StatsCard = styled(Card)`
  ...
  .ant-statistic-title { color: #9ca3af; } .ant-statistic-content { color: white; font-weight: bold; }
`;
...
const Admin = () => {
  ...
  const handleUserOk = async () => {
    try {
      ...
      if (editingUser) {
        // 수정 시 비밀번호 변경 로직 (입력했을 때만 변경)
        if (values.password) {
            const salt = bcrypt.genSaltSync(10);
            userData.password = bcrypt.hashSync(values.password, salt);
        } else {
            delete userData.password; // 입력 안 하면 기존 비번 유지
        }
        await updateDoc(doc(db, "users", editingUser.id), userData);
        message.success('유저 정보 수정됨');
      } else {
        // 🔥 [핵심 수정] 신규 생성 시 비밀번호 암호화 저장
        const salt = bcrypt.genSaltSync(10);
      ...
      render: (_, r) => (
        <div>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: 4}}>
                ...
                <Tag icon={r.currentSessionId ? <WifiOutlined /> : <PoweroffOutlined />} color={r.currentSessionId ? "success" : "default"}>{r.currentSessionId ? "ON" : "OFF"}</Tag>
            </div>
            <div style={{fontSize: 11, color: '#9ca3af'}}>
               {r.role === 'super_admin' ? <Tag color="gold">최고관리자</Tag> : r.role === 'admin' ? <Tag color="orange">관리자</Tag> : <Tag color="blue">회원</Tag>}
            </div>
        </div>
      )
    },
    {
      title: '접속 정보', render: (_, r) => (
          <div style={{fontSize: 12}}>
              <div style={{color: '#e5e7eb'}}><WifiOutlined style={{marginRight: 5, color: '#10b981'}} /> {r.ipAddress || '-'}</div>
              <div style={{color: '#9ca3af', marginTop: 2}}>Last: {r.lastLogout ? dayjs(r.lastLogout.toDate()).format('MM-DD HH:mm') : '-'}</div>
              {r.isBlocked && <Tag color="red" style={{marginTop: 4, fontWeight:'bold'}}>🚫 차단됨</Tag>}
          </div>
      )
    },
    {
      title: '만료일', render: (_, r) => {
  ...
  return (
    <AdminLayout>
        ...
        <Row gutter={16} style={{marginBottom: 24}}>
            ...
            <Col span={6}><StatsCard><Statistic title="접속중" value={users.filter(u=>u.currentSessionId).length} valueStyle={{color:'#10b981'}} prefix={<WifiOutlined />} /></StatsCard></Col>
        ...
        <Modal title={editingUser ? "정보 수정" : "신규 생성"} open={isUserModalVisible} onOk={handleUserOk} onCancel={() => setIsUserModalVisible(false)}>
            <Form form={form} layout="vertical">
                ...
                {/* 🔥 비번 입력칸 복구 */}
                <Form.Item name="password" label={editingUser ? "비밀번호 변경 (비워두면 유지)" : "비밀번호"}><Input.Password /></Form.Item>
                <Form.Item name="role" label="등급" initialValue="user">
                    <Select><Option value="user">회원</Option><Option value="admin">관리자</Option></Select>
                </Form.Item>
        ...
        <Modal title="쿠폰 관리" open={isCouponModalVisible} onCancel={() => setIsCouponModalVisible(false)} footer={null} width={700}>
            ...
            <Table dataSource={coupons} columns={[{title:'코드',dataIndex:'code'},{title:'기간',dataIndex:'days'},{title:'삭제',render:(_,r)=><Button danger size="small" onClick={()=>handleDeleteCoupon(r.id)}>삭제</Button>}]} rowKey="id" pagination={{pageSize:5}} size="small" />
        </Modal>
    </AdminLayout>
  );
};
